...
#include "appvalidate.h" // appvalidate_verify
...
{
    // App code starts after metadata page
    uint32_t *app = (void*)(CONFIG_LAUNCH_APP_ADDRESS + CONFIG_METADATA_PAGE_SIZE);

    // Quick sanity check: first word must be valid stack pointer
    if (*app == 0 || *app == 0xffffffff)
        return 0;

    // Full validation via hash (metadata generated by app build system)
...
boot_start_application(void)
{
    ...
    // App code starts after metadata page
    uint32_t *vtor = (void*)(CONFIG_LAUNCH_APP_ADDRESS + CONFIG_METADATA_PAGE_SIZE);
